...
// /api/scan/info and lets the owner start a barcode-scanner pairing session.
(function () {
  ...
  const pairResult = $('connect-pair-result');
  const pairCodeEl = $('connect-pair-code');
  const scanUrlInput = $('connect-scan-url');
  ...
  $('connect-copy-scan').addEventListener('click', async () => {
    if (scanUrlInput.value && await copyText(scanUrlInput.value)) {
      statusEl.textContent = I18N.t('connect.copied');
    }
  });
  ...
  $('connect-start-pair').addEventListener('click', async () => {
    try {
      const res = await fetch('/api/scan/start', { method: 'POST' });
      const data = await res.json();
      const scanUrl = scanBaseUrl ? scanBaseUrl + '?pair=' + encodeURIComponent(data.code) : '';
      pairCodeEl.textContent = data.code;
      scanUrlInput.value = scanUrl;
      const pairQr = $('connect-pair-qr');
      if (pairQr) makeQr(pairQr, $('connect-pair-qr-fallback') || qrFallback, scanUrl);
      pairResult.hidden = false;
      statusEl.textContent = '';
    } catch (e) {
      statusEl.textContent = I18N.t('connect.pairError');
    }
  });
