...
// status, job title / phone / hire date) on top of the existing accounts table.
// Owner only. New workers get the 'worker' role so they can self-clock.

const form = document.getElementById('staff-form');
const listEl = document.getElementById('staff-list');
const nameInput = document.getElementById('staff-name');
const pinInput = document.getElementById('staff-pin');
const jobInput = document.getElementById('staff-job');
...
function roleLabel(w) {
  if (w.role === 'owner') return I18N.t('role.owner');
  if (w.role === 'worker') return I18N.t('role.worker');
  ...
  listEl.innerHTML = staffCache.length ? `
    <table class="product-table">
      <thead><tr>
        ...
        <th>${I18N.t('staff.thJob')}</th>
        <th>${I18N.t('staff.thPhone')}</th>
        <th>${I18N.t('staff.thHired')}</th>
      ...
      <tbody>
        ${staffCache.map(w => `
          <tr>
            ...
            <td>${escapeHtml(payTypeLabel(w))}${w.hourly_rate > 0 ? `<div class="hint-text">${formatMoney(w.hourly_rate)}/h</div>` : ''}${w.monthly_salary > 0 ? `<div class="hint-text">${formatMoney(w.monthly_salary)}/mo</div>` : ''}</td>
...
form.addEventListener('submit', async (e) => {
  ...
  // Save pay fields + profile + active flag.
  ...
  // New worker: create a 'worker' account (PIN auto-generated when left blank),
  // then attach the pay/profile fields.
  ...
  const res = await fetch('/api/users', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ name, pin, role: 'worker', permissions: ['pointage'] })
  });
  if (!res.ok) {
    alert(I18N.t('inv.error') + ' ' + I18N.serverError((await res.json()).error));
    return;
