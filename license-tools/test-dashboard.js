...
setTimeout(async () => {
  try {
    ...
    check('issued key defaults to PRO tier', issue.data.tier === 'pro');
    const lic = issue.data;

    const issueBasic = await req('/api/licenses', { method: 'POST', body: JSON.stringify({ client: 'Pharmacy Gamma', machineId: 'bbb222', tier: 'basic' }), headers: auth });
    check('issue basic license', issueBasic.status === 201 && issueBasic.data.tier === 'basic');
    const verifyBasic = appLic.verifyLicense(issueBasic.data.key, new Date('2026-08-07').getTime());
    check('basic key carries tier in payload', verifyBasic.payload.tier === 'basic');

    const badTier = await req('/api/licenses', { method: 'POST', body: JSON.stringify({ client: 'X', tier: 'ultra' }), headers: auth });
    check('invalid tier rejected', badTier.status === 400);
    ...
    check('list returns 3 licenses', list.data.length === 3);
